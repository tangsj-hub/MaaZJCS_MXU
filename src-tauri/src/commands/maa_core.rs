...
// 与 MaaToolkit 默认策略保持一致的保守真机默认值。
const DEFAULT_ADB_SCREENCAP_METHODS: u64 = 71;
const DEFAULT_ADB_INPUT_METHODS: u64 = 7;
...
fn map_toolkit_adb_devices(devices: Vec<maa_framework::toolkit::AdbDevice>) -> Vec<AdbDevice> {
    devices
        .into_iter()
        .map(|d| {
            to_adb_device(
                d.name,
                d.adb_path.to_string_lossy().to_string(),
                d.address,
                d.screencap_methods,
                d.input_methods,
                d.config.to_string(),
...
fn normalize_adb_path_key(path: &str) -> String {
...
fn merge_adb_devices(
    toolkit_devices: Vec<AdbDevice>,
    fallback_devices: Vec<AdbDevice>,
) -> Vec<AdbDevice> {
    let mut merged = toolkit_devices;
    let mut seen: HashSet<(String, String)> = merged
        .iter()
        .map(|device| {
            (
                normalize_adb_path_key(&device.adb_path),
                device.address.to_lowercase(),
            )
        })
        .collect();
...
fn candidate_adb_paths(adb_executable_path: Option<&str>) -> Vec<PathBuf> {
    let mut candidates = Vec::new();
    let mut seen = HashSet::new();
    let mut push_candidate = |adb_path: PathBuf| {
        if adb_path.exists() {
            let key = normalize_adb_path_key(&adb_path.to_string_lossy());
            if seen.insert(key) {
                candidates.push(adb_path);
            }
        }
    };

    if let Some(path) = adb_executable_path {
...
fn parse_adb_devices_output(output: &str, adb_path: &Path) -> Vec<AdbDevice> {
    output
        .lines()
        .filter_map(|line| {
            ...
            Some(to_adb_device(
                ...
                address.to_string(),
                DEFAULT_ADB_SCREENCAP_METHODS,
                DEFAULT_ADB_INPUT_METHODS,
                "{}".to_string(),
            ))
        })
        .collect()
...
fn find_adb_devices_fallback(adb_executable_path: Option<&str>) -> Vec<AdbDevice> {
    let mut fallback_devices = Vec::new();
    let mut seen = HashSet::new();
    let candidate_paths = candidate_adb_paths(adb_executable_path);

    info!(
        "ADB fallback probing {} candidate path(s): {:?}",
        candidate_paths.len(),
        candidate_paths
    );

    for adb_path in candidate_paths {
        let output = match Command::new(&adb_path).arg("devices").output() {
            Ok(output) => output,
            Err(err) => {
                warn!("Failed to run adb devices via {:?}: {}", adb_path, err);
                continue;
            }
        };
        ...
        if !output.status.success() {
            ...
            warn!(
                "adb devices returned non-zero status via {:?}: {}",
                adb_path, stderr
            );
            continue;
        }

        for device in parse_adb_devices_output(&String::from_utf8_lossy(&output.stdout), &adb_path)
        {
            let key = (
                normalize_adb_path_key(&device.adb_path),
                device.address.to_lowercase(),
            ...
            if seen.insert(key) {
                fallback_devices.push(device);
            }
        }
    }

    info!("ADB fallback discovered {} device(s)", fallback_devices.len());
...
pub async fn maa_find_adb_devices(
    ...
    adb_executable_path: Option<String>,
    ...
    tauri::async_runtime::spawn_blocking(move || {
        let toolkit_devices = match Toolkit::find_adb_devices() {
            Ok(devices) => map_toolkit_adb_devices(devices),
            Err(err) => {
                warn!("Toolkit::find_adb_devices failed: {}", err);
                Vec::new()
            }
        };

        let fallback_devices = if adb_executable_path
            .as_deref()
            .map(|path| !path.trim().is_empty())
            .unwrap_or(false)
        {
            find_adb_devices_fallback(adb_executable_path.as_deref())
        } else {
            info!("ADB executable path not configured, using Toolkit-only detection");
            Vec::new()
        };
        let result_devices = merge_adb_devices(toolkit_devices, fallback_devices);
